...
test('renders same number of options seasons are passed in', ()=>{
    render(<Show show={testShow} selectedSeason={'none'}/>)
    const numOption= screen.getAllByTestId('season-option')
    expect(numOption).toHaveLength(2)
});

test('handleSelect is called when an season is selected', () => {
    render(<Show show={testShow} selectedSeason={testShow.seasons[0]}/>)
    userEvent.selectOptions(screen.get('select'),['0'])
    expect(screen.getByRole('option',{name:"season1"}).selected).toBe(true)
});
